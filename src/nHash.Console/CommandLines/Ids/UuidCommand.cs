...
public class UuidCommand(IOutputProvider outputProvider, IUuidService uuidService) : IUuidCommand
{
    ...
    {
        var command = new BaseCommand("uuid", "Generate a Universally unique identifier (UUID/GUID) version 1 to 8, ULID, or NanoID",
            GetExamples());
    ...
    private static List<KeyValuePair<string, string>> GetExamples() =>
        [
            new("Generate a version 7 time-ordered UUID", "nhash uuid -v v7"),
            new("Generate a ULID", "nhash uuid -v ulid"),
            new("Generate all versions of UUIDs and save the output to a file", "nhash uuid --output uuid.txt"),
        ];
